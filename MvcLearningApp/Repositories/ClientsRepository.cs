    ...
    public class ClientsRepository
    {
        ...
        public GridViewModel<ClientView> Compute(int? page, string words, int idUser, int rowPerPage = 5, int pagerLength = 5)
        {
            ...
            if (!string.IsNullOrEmpty(words))
            {
                ...
                //echivalent cu 
                //queryC = words.Split(' ').Aggregate(queryC, (current, word) => (from c in current where c.Name.Contains(word) || c.Email.Contains(word) select c));
            }

            int totalRecords = queryC.Count();
            if (totalRecords > 0)
            {
                result.Pager.Init(totalRecords, rowPerPage, pagerLength, page);
        ...
        public bool DeleteClient(int? id)
        {
            ...
            db.Clients.Remove((from x in db.Clients where x.Id == id select x).FirstOrDefault());
            db.SaveChanges();
            return true;
        }
